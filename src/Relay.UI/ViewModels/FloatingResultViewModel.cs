...
public partial class FloatingResultViewModel : ObservableObject
{
    ...
    private async Task RunAnalysisAsync(string? userQuestion)
    {
        ...
        Title = string.IsNullOrWhiteSpace(userQuestion) ? "Analyzing Selection..." : "Thinking...";
        ...
        try
        {
            ...
            // Post-analysis fallback and title guarantee
            if (!HasError)
            {
                if (string.IsNullOrWhiteSpace(Title) || Title == "Analyzing Selection..." || Title == "Thinking...")
                {
                    Title = !string.IsNullOrWhiteSpace(Summary)
                        ? Summary
                        : (!string.IsNullOrWhiteSpace(userQuestion)
                            ? userQuestion
                            : (!string.IsNullOrWhiteSpace(ApplicationBadge) && ApplicationBadge != "Screen Selection"
                                ? $"{ApplicationBadge} Overview"
                                : "Screen Analysis"));
                }
                ...
                {
                    HasError = true;
                    ErrorMessage = "No response was generated by the AI model. Please verify your provider settings or try again.";
                    Title = "Notice";
                }
            }
            ...
            if (!HasError && _settingsService.CurrentSettings.SaveHistory)
            {
                try
                {
                    ...
                    string resolvedTitle = !string.IsNullOrWhiteSpace(Title) && Title != "Analyzing Selection..." && Title != "Thinking..."
                        ? Title
